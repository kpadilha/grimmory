...
describe('MetadataReviewDialogComponent', () => {
  ...
  const booksById = new Map([[11, {id: 11, title: 'Book 11'}]]);
  const getBooksByIds = vi.fn((bookIds: number[]) =>
    Promise.resolve(bookIds.map(id => booksById.get(id)).filter((book): book is {id: number; title: string} => !!book))
  );
  ...
  it('loads proposals for the requested task and resolves the current book from /books/batch', async () => {
